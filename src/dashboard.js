...
function postTable(p,rows){return `<table><thead><tr><th scope="col">#</th><th scope="col">Published</th><th scope="col">Format</th><th scope="col">${p.id==="podcast"?"Episode":"Post"}</th><th class="n" scope="col">${unit(p)}</th><th class="n" scope="col">${p.id==="podcast"?"First 30 days":"Engagements"}</th></tr></thead><tbody>${rows.map((r,i)=>`<tr><td class="n">${i+1}</td><td style="white-space:nowrap">${date(r.date)}</td><td>${esc(r.category)}</td><td class="table-title">${titleLink(r)}</td><td class="n">${full(r.value)}</td><td class="n">${full(p.id==="podcast"?r.downloads_first_30_days:r.engagements)}</td></tr>`).join("")}</tbody></table>`}
...
$("definitions").innerHTML=PLATS.map(p=>`<div class="card"><h3>${dot(p)}${p.label}</h3><div class="definition"><b>Metric</b><span>${p.id==="podcast"?"All-player podcast downloads from the Substack export. Downloads are not unique audience. First 30 days shows downloads during each episode’s first 30 days, as reported in the export.":p.id==="instagram"?"Exported post views. Engagements = likes + comments + shares + saves.":"Exported video views. Engagements = likes + comments + shares."}</span><b>Data date</b><span>${cutoffText(p)}</span><b>Coverage</b><span>${esc(p.coverage_note)}</span></div></div>`).join("");
