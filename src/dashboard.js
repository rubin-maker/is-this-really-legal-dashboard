...
const unit = p => p.id==="apple"?"plays":"views";
const coveredEnd = (p,row) => row.end > p.cutoff ? p.cutoff : row.end;
const contentUnit = p => p.id==="apple"?"episodes":"posts";
...
const safeUrl = u => /^https:\/\/(www\.)?(youtube\.com|instagram\.com|podcasts\.apple\.com)\//.test(u||"")?u:null;
...
const metricLabel = (p,m) => m==="value"?unit(p):m==="posts"?contentUnit(p):"engagements";
...
$("coverageBanner").textContent = "YouTube: September 17 export. Apple Podcasts and Instagram: August 28 snapshots; updated exports pending. YouTube’s selected analytics date range is not included in its CSVs.";
const yt=byId.youtube, ig=byId.instagram, ap=byId.apple;
$("hero").innerHTML = [
 ["YouTube views",yt.totals.weekly_value??sum(records(yt),"value"),"41 dated videos · Sep 17 export"],
 ["Instagram views",ig.totals.value,"20 posts · Aug 28 snapshot"],
 ["Apple Podcasts plays",ap.totals.value,"10 episodes · Aug 28 snapshot"],
 ["YouTube subscribers gained",sum(records(yt),"subscribers_gained"),"Known gains · 1 video missing this metric"],
...
function postTable(p,rows){return `<table><thead><tr><th scope="col">#</th><th scope="col">Published</th><th scope="col">Format</th><th scope="col">${p.id==="apple"?"Episode":"Post"}</th><th class="n" scope="col">${unit(p)}</th><th class="n" scope="col">${p.id==="apple"?"Listeners":"Engagements"}</th></tr></thead><tbody>${rows.map((r,i)=>`<tr><td class="n">${i+1}</td><td style="white-space:nowrap">${date(r.date)}</td><td>${esc(r.category)}</td><td class="table-title">${titleLink(r)}</td><td class="n">${full(r.value)}</td><td class="n">${full(p.id==="apple"?r.listeners:r.engagements)}</td></tr>`).join("")}</tbody></table>`}
...
$("subscribers").innerHTML=`<div><div class="label">Exported video subscriber gains</div><div class="value">${full(subTotal)}</div><div class="detail">${cutoffText(yt)}</div></div><div>${full(sum(ytRecords.filter(r=>r.category==="Long-form"),"subscribers_gained"))} from long-form · ${full(sum(ytRecords.filter(r=>r.category==="Shorts"),"subscribers_gained"))} from Shorts<div class="detail">Uses the CSV’s Subscribers gained field. One dated video has no value; totals sum the available values.</div></div>`;
function renderSubscriber(){const ww=yt.weekly.filter(r=>covered(r)&&($("subscriberRange").value==="all"||!r.partial)).slice($("subscriberRange").value==="all"?0:-8);$("subscriberWeekly").innerHTML=`<table><thead><tr><th>Publish week</th><th class="n">Long-form</th><th class="n">Shorts</th><th class="n">Total gains</th><th>Coverage</th></tr></thead><tbody>${ww.map(w=>`<tr><th>${span(w.start,coveredEnd(yt,w))}, 2026</th><td class="n">${full(sum(weekRows(yt,w.start,"Long-form"),"subscribers_gained"))}</td><td class="n">${full(sum(weekRows(yt,w.start,"Shorts"),"subscribers_gained"))}</td><td class="n">${full(sum(weekRows(yt,w.start),"subscribers_gained"))}</td><td>${status(yt,w.start)}</td></tr>`).join("")}</tbody></table>`}
$("subscriberRange").onchange=renderSubscriber;

function renderCategories(id){const p=byId[id];document.querySelectorAll("#categoryTabs button").forEach(b=>b.classList.toggle("on",b.dataset.platform===id));$("categories").innerHTML=`<table><thead><tr><th>Format</th><th class="n">${contentUnit(p)}</th><th class="n">${unit(p)}</th><th class="n">Average per item</th><th class="n">${p.id==="apple"?"Median episode listeners":"Engagements"}</th></tr></thead><tbody>${cats(p).map(c=>{const rr=records(p).filter(r=>r.category===c),v=sum(rr,"value"),listeners=rr.map(r=>r.listeners).filter(v=>v!=null).sort((a,b)=>a-b),mid=Math.floor(listeners.length/2),median=listeners.length?(listeners.length%2?listeners[mid]:(listeners[mid-1]+listeners[mid])/2):null;return `<tr><td>${esc(c)}</td><td class="n">${rr.length}</td><td class="n">${full(v)}</td><td class="n">${fmt(v/rr.length)}</td><td class="n">${full(p.id==="apple"?median:sum(rr,"engagements"))}</td></tr>`}).join("")}</tbody></table>`}
...
$("definitions").innerHTML=PLATS.map(p=>`<div class="card"><h3>${dot(p)}${p.label}</h3><div class="definition"><b>Metric</b><span>${p.id==="apple"?"Apple plays. Unique listeners are shown per episode and never added across episodes.":p.id==="instagram"?"Exported post views. Engagements = likes + comments + shares + saves.":"Exported video views. Engagements = likes + comments + shares."}</span><b>Data date</b><span>${cutoffText(p)}</span><b>Coverage</b><span>${esc(p.coverage_note)}</span></div></div>`).join("");
